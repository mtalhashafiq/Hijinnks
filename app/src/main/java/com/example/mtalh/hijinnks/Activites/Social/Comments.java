...
import com.example.mtalh.hijinnks.Models.Model_Comments;
import com.example.mtalh.hijinnks.R;
import com.example.mtalh.hijinnks.Adapter.RecyclerAdapter.RecyclerAdapter_comment;
...
ImageView back_button_comments;
    ...
    RecyclerAdapter_comment adapter;
        ...
        comment_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!comment_text.getText().toString().isEmpty()) {
                    list_comments.add(new Model_Comments("Leonard Perze", "5 days ago ", comment_text.getText().toString()));
                    InputMethodManager imm = (InputMethodManager) getSystemService(getApplicationContext().INPUT_METHOD_SERVICE);
                    imm.hideSoftInputFromWindow(view.getWindowToken(), 0);
                    adapter.notifyItemInserted(list_comments.size() - 1);
